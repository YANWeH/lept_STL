/*************************************************************************
	...
	> Created Time: Thu 11 Aug 2022 09:16:17 AM EDT
 ************************************************************************/

#ifndef LEPTSTL_LIST_H__
#define LEPTSTL_LIST_H__ 
...
namespace leptstl 
{
    ...
    template<typename T>
        struct list_node : public list_node_base<T>
        {
            ...
            list_node(T&& v) : value(leptstl::move(v)) {}
    ...
    /* list 迭代器设计*/
    template<typename T>
        struct list_iterator : public leptstl::iterator<leptstl::bidirectional_iterator_tag, T>
        {
    ...
    template<typename T>
        class list 
        {
            public:
                ...
                typedef leptstl::allocator<list_node_base<T>>      base_allocator;
                typedef leptstl::allocator<list_node<T>>           node_allocator;
...
/*******************************************************************************************/
                ...
                template <typename ...Args>
                    void     emplace_front(Args&& ...args)
                    {
                        ...
                        auto link_node = create_node(leptstl::forward<Args>(args)...);
...
/*******************************************************************************************/
                ...
                {
                  THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
                  auto link_node = create_node(leptstl::move(value));
                  ++size_;
                  return link_iter_node(pos, link_node->as_base());
                }
...
/*******************************************************************************************/
                ...
                void unique()
                { unique(leptstl::equal_to<T>()); }
                template <class BinaryPredicate>
                    void unique(BinaryPredicate pred);

                void merge(list& x)
                { merge(x, leptstl::less<T>()); }
                template <typename Compared>
                    void merge(list& x, Compared comp);

                void sort()
                { list_sort(begin(), end(), size(), leptstl::less<T>()); }
                template <typename Compared>
...
/*******************************************************************************************/
    ...
    template<typename T>
        ...
        {
            LEPTSTL_DEBUG(this != &x);
            if(!x.empty())
            {
                THROW_LENGTH_ERROR_IF(size_ > max_size() - x.size_, " list<T>'s size to big");
...
/*******************************************************************************************/
        ...
        template <typename T>
            void list<T>::reverse()
            {
                ...
                {
                    leptstl::swap(i.node_->prev, i.node_->next);
                    i.node_ = i.node_->prev;
                }
                leptstl::swap(e.node_->prev, e.node_->next);
            }
...
/*******************************************************************************************/
        ...
        template <typename T>
            ...
            list<T>::create_node(Args&& ...args)
            {
                ...
                try
                {
                    data_allocator::construct(leptstl::address_of(p->value), leptstl::forward<Args>(args)...);
                    p->prev = nullptr;
                    p->next = nullptr;
                }
                catch (...)
...
/*******************************************************************************************/
        ...
        template <typename T>
            ...
            {
                data_allocator::destroy(leptstl::address_of(p->value));
                node_allocator::deallocate(p);
            }
...
/*******************************************************************************************/
        ...
        template <typename T>
            bool operator<(const list<T>& lhs, const list<T>& rhs)
            {
              return leptstl::lexicographical_compare(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
            }
