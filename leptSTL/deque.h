/*************************************************************************
	...
	> Created Time: Fri 12 Aug 2022 09:40:14 AM EDT
 ************************************************************************/

#ifndef LEPTSTL_DEQUE_H__
#define LEPTSTL_DEQUE_H__ 
...
#define DEQUE_MAP_INIT_SIZE 8
    ...
    template<typename T>
        class deque 
        {
            public:
                ...
                typedef leptstl::allocator<T*>                      map_allocator;
            ...
            public:
                ...
                template <typename IIter, typename std::enable_if<
                        leptstl::is_input_iterator<IIter>::value, int>::type = 0>
                    deque(IIter first, IIter last)
                    { copy_init(first, last, iterator_category(first)); }
              
                deque(std::initializer_list<value_type> ilist)
                {
                    copy_init(ilist.begin(), ilist.end(), leptstl::forward_iterator_tag());
                }
              
                deque(const deque& rhs)
                {
                    copy_init(rhs.begin(), rhs.end(), leptstl::forward_iterator_tag());
                }
                deque(deque&& rhs) noexcept
                    :begin_(leptstl::move(rhs.begin_)),
                    end_(leptstl::move(rhs.end_)),
                    map_(rhs.map_),
            ...
            public:
                ...
                { copy_assign(ilist.begin(), ilist.end(), leptstl::forward_iterator_tag{}); }
                ...
                void     push_front(value_type&& value) { emplace_front(leptstl::move(value)); }
                void     push_back(value_type&& value)  { emplace_back(leptstl::move(value)); }
    ...
    template <typename T>
        deque<T>& deque<T>::operator=(const deque& rhs)
        {
            if (this != &rhs)
            {
                ...
                if (len >= rhs.size())
                {
                    erase(leptstl::copy(rhs.begin_, rhs.end_, begin_), end_);
                }
                else
                {
                    iterator mid = rhs.begin() + static_cast<difference_type>(len);
                    leptstl::copy(rhs.begin_, mid, begin_);
    ...
    template <typename T>
        ...
        void deque<T>::emplace_front(Args&& ...args)
        {
            ...
            {
                data_allocator::construct(begin_.cur - 1, leptstl::forward<Args>(args)...);
                --begin_.cur;
            }
            else
            {
                ...
                {
                    --begin_;
                    data_allocator::construct(begin_.cur, leptstl::forward<Args>(args)...);
                }
                catch (...)
                {
                    ++begin_;
    ...
    template <typename T>
        ...
        void deque<T>::emplace_back(Args&& ...args)
        {
            ...
            {
                data_allocator::construct(end_.cur, leptstl::forward<Args>(args)...);
                ++end_.cur;
            }
            else
            {
    ...
    template <typename T>
        ...
        typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&& ...args)
        {
            ...
            {
                emplace_front(leptstl::forward<Args>(args)...);
                return begin_;
            }
            else if (pos.cur == end_.cur)
            {
                emplace_back(leptstl::forward<Args>(args)...);
                return end_;
            }
            return insert_aux(pos, leptstl::forward<Args>(args)...);
        }
    ...
    template <typename T>
        ...
        deque<T>::insert(iterator position, value_type&& value)
        {
            if (position.cur == begin_.cur)
            {
                emplace_front(leptstl::move(value));
                return begin_;
            }
            else if (position.cur == end_.cur)
            {
            ...
            {
                return insert_aux(position, leptstl::move(value));
            }
        }
    ...
    template <typename T>
        void deque<T>::insert(iterator position, size_type n, const value_type& value)
        {
            if (position.cur == begin_.cur)
            {
                ...
                leptstl::uninitialized_fill_n(new_begin, n, value);
                begin_ = new_begin;
            }
            else if (position.cur == end_.cur)
            {
                ...
                auto new_end = end_ + n;
                leptstl::uninitialized_fill_n(end_, n, value);
                end_ = new_end;
            }
            else
            {
    ...
    template <typename T>
        ...
        deque<T>::erase(iterator position)
        {
            ...
            {
                leptstl::copy_backward(begin_, position, next);
                pop_front();
            }
            else
            {
                leptstl::copy(next, end_, position);
                pop_back();
            }
    ...
    template <typename T>
        ...
        deque<T>::erase(iterator first, iterator last)
        {
            ...
            else
            {
                ...
                {
                    leptstl::copy_backward(begin_, first, last);
    ...
    template <typename T>
        void deque<T>::clear()
        {
            ...
            { /* 有两个以上的缓冲区*/
                leptstl::destroy(begin_.cur, begin_.last);
                leptstl::destroy(end_.first, end_.cur);
            }
            else
            {
                leptstl::destroy(begin_.cur, end_.cur);
            }
            end_ = begin_;
            shrink_to_fit();
        }
    ...
    template <typename T>
        void deque<T>::swap(deque& rhs) noexcept
        {
            if (this != &rhs)
            {
                ...
                leptstl::swap(map_, rhs.map_);
                leptstl::swap(map_size_, rhs.map_size_);
            }
        }
    ...
    template <typename T>
        void deque<T>::map_init(size_type nElem)
        {
            ...
            map_size_ = leptstl::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
            try
            {
    ...
    template <typename T>
        void deque<T>::fill_init(size_type n, const value_type& value)
        {
            ...
            if (n != 0)
            {
                ...
                {
                    leptstl::uninitialized_fill(*cur, *cur + buffer_size, value);
                }
                leptstl::uninitialized_fill(end_.first, end_.cur, value);
            }
        }
    ...
    template <typename T>
        ...
        void deque<T>::copy_init(FIter first, FIter last, forward_iterator_tag)
        {
            ...
            {
                auto next = first;
                leptstl::advance(next, buffer_size);
                leptstl::uninitialized_copy(first, next, *cur);
                first = next;
            }
            leptstl::uninitialized_copy(first, last, end_.first);
        }
    ...
    template <typename T>
        ...
        {  
            ...
            const size_type len2 = leptstl::distance(first, last);
            if (len1 < len2)
            {
                auto next = first;
                leptstl::advance(next, len1);
                leptstl::copy(first, next, begin_);
                insert_dispatch(end_, next, last, forward_iterator_tag{});
            }
            else
            {
                erase(leptstl::copy(first, last, begin_), end_);
            }
        }
    ...
    template <typename T>
        ...
        {
            const size_type elems_before = position - begin_;
            value_type value_copy = value_type(leptstl::forward<Args>(args)...);
            if (elems_before < (size() / 2))
            { /* 在前半段插入*/
                emplace_front(front());
                auto front1 = begin_;
                ...
                auto pos = position;
                ++pos;
                leptstl::copy(front2, pos, front1);
            }
            else
            { /* 在后半段插入*/
                emplace_back(back());
                ...
                --back2;
                position = begin_ + elems_before;
                leptstl::copy_backward(position, back2, back1);
            }
            *position = leptstl::move(value_copy);
            return position;
        }
        ...
        void deque<T>::fill_insert(iterator position, size_type n, const value_type& value)
        {
            ...
            if (elems_before < (len / 2))
            {
                ...
                try
                {
                    if (elems_before >= n)
                    {
                        ...
                        leptstl::uninitialized_copy(begin_, begin_n, new_begin);
                        begin_ = new_begin;
                        leptstl::copy(begin_n, position, old_begin);
                        leptstl::fill(position - n, position, value_copy);
                    }
                    else
                    {
                        leptstl::uninitialized_fill(
                        leptstl::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
                        begin_ = new_begin;
                        leptstl::fill(old_begin, position, value_copy);
                    }
                }
                catch (...)
                {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        auto end_n = end_ - n;
                        leptstl::uninitialized_copy(end_n, end_, end_);
                        end_ = new_end;
                        leptstl::copy_backward(position, end_n, old_end);
                        leptstl::fill(position, position + n, value_copy);
                }
                    else
                    {
                        leptstl::uninitialized_fill(end_, position + n, value_copy);
                        leptstl::uninitialized_copy(position, end_, position + n);
                        end_ = new_end;
                    leptstl::fill(position, old_end, value_copy);
                    }
                }
                catch (...)
                {
            ...
            if (elems_before < (len / 2))
            {
                ...
                try
                {
                    if (elems_before >= n)
                    {
                        ...
                        begin_ = new_begin;
                        leptstl::copy(begin_n, position, old_begin);
                        leptstl::copy(first, last, position - n);
                    }
                    else
                    {
                        auto mid = first;
                        leptstl::advance(mid, n - elems_before);
                        leptstl::uninitialized_copy(first, mid,
                                              leptstl::uninitialized_copy(begin_, position, new_begin));
                        begin_ = new_begin;
                        leptstl::copy(mid, last, old_begin);
                    }
                }
                catch (...)
                {
            ...
            else
            {
                ...
                try
                {
                    if (elems_after > n)
                    {
                        ...
                        end_ = new_end;
                        leptstl::copy_backward(position, end_n, old_end);
                        leptstl::copy(first, last, position);
                    }
                    else
                    {
                        auto mid = first;
                        leptstl::advance(mid, elems_after);
                    leptstl::uninitialized_copy(position, end_,
                                            leptstl::uninitialized_copy(mid, last, end_));
                    end_ = new_end;
                    leptstl::copy(first, mid, position);
                    }
                }
                catch (...)
                {
    ...
    template <typename T>
        ...
        void deque<T>::insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag)
        {
            ...
            if (position.cur == begin_.cur)
            {
                ...
                try
                {
                    leptstl::uninitialized_copy(first, last, new_begin);
                    begin_ = new_begin;
                }
                catch (...)
                {
            ...
            else if (position.cur == end_.cur)
            {
                ...
                try
                {
                    leptstl::uninitialized_copy(first, last, end_);
                    end_ = new_end;
                }
                catch (...)
                {
    ...
    template <typename T>
        ...
        {
            const size_type new_map_size = leptstl::max(map_size_ << 1,
    ...
    template <typename T>
        bool operator==(const deque<T>& lhs, const deque<T>& rhs)
        {
            return lhs.size() == rhs.size() && 
                    leptstl::equal(lhs.begin(), lhs.end(), rhs.begin());
        }
        
    template <typename T>
        bool operator<(const deque<T>& lhs, const deque<T>& rhs)
        {
            return leptstl::lexicographical_compare(
